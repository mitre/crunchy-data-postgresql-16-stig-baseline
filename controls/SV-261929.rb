control 'SV-261929' do
  title "PostgreSQL must only accept end entity certificates issued by #{input('org_name')[:acronym]} PKI or #{input('org_name')[:acronym]}-approved PKI Certification Authorities (CAs) for the establishment of all encrypted sessions."
  desc "Only #{input('org_name')[:acronym]}-approved external PKIs have been evaluated to ensure that they have security controls and identity vetting procedures in place which are sufficient for #{input('org_name')[:acronym]} systems to rely on the identity asserted in the certificate. PKIs lacking sufficient security controls and identity vetting procedures risk being compromised and issuing certificates that enable adversaries to impersonate legitimate users.

The authoritative list of #{input('org_name')[:acronym]}-approved PKIs is published at https://public.cyber.mil/pki-pke/interoperability/.
